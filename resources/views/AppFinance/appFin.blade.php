...
@section('content')
    ...
    @endforeach
    @foreach ($pettycash as $a)
        <div class="modal fade" id="modalPencairan/{{ $a->id }}" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header no-bd">
                        <h5 class="modal-title">
                            <span class="fw-largebold"> Form Pencairan</span>
                        </h5>
                    ...
                    <form action="{{ route('paid', ['id' => $a->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="">Tanggal Pencairan</label>
                                <input type="date" class="form-control" id="tanggal_pencairan" required
        ...
        $('.btn-approve').click(function(e) {
            ...
            swal({
                ...
                if (willApprove) {
                    ...
                    form.attr('action', '/pettycash/approvedFinance/' + id);
                    form.submit();
                }
            });
        });
        $('.btn-reject').click(function(e) {
            ...
            swal({
                ...
                if (willReject) {
                    ...
                    // submit form ke route delete
                    var form = $('#formReject');
                    form.attr('action', '/pettycash/rejectedFinance/' + id);
                    form.submit();
                }
            });
        });
